...
local ID_WHORED_SINGLE		= 2
local ID_COOP_OPTIONS		= 3
...
function main_menu_coop_top_set_menu()
	...
	if Whored_mode_active == true then 
		...
		Data[1] = Whored_single_button
		Data[2] = Xbox_live_button
		Data[3] = Coop_options_button
	else
		header_str = "MAINMENU_COOP"
		Data[1] = Xbox_live_button
		Data[2] = Coop_options_button
	end
...
function main_menu_coop_top_button_a(event, acceleration)
	if Tween_done == true then
		...
		if game_get_platform() == "PS4" and Whored_mode_active and current_id ~= ID_WHORED_SINGLE then
			if game_is_connected_to_network() == true and game_is_connected_to_service() == false then
			...
			if main_menu_check_needs_patch() == true then --check patch and show msg
				return
			end
			if game_user_have_ps_plus(true) == false then --check ps plus and show msg
				return
			end
		end
